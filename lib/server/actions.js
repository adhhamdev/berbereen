...
export const signUpWithEmail = async (formData) => {
  ...
  redirect("/?action=signup&auth=email", "replace");
};
...
export const signInWithEmail = async (formData) => {
  ...
  redirect("/?action=login&auth=email", "replace");
};
...
export const updatePrefs = async (changes) => {
  try {
    ...
    console.log(`User ${user.$Id} updated:`, updatedPrefs);
  } catch (error) {
    console.error("Error updating user:", error);
  }
};
...
export async function deleteCurrentUser() {
  ...
  redirect("/login?action=delete-user");
}
