...
export const createProfile = async (formData) => {
  ...
  if (user.name != formData.get("name")) {
    await account.updateName(formData.get("name"));
  }

  await setAvatar(user, account);
...
export const setAvatar = async (user, account) => {
...
export const getAvatar = async (size) => {
  ...
  if (user) {
    try {
      const { storage } = createStorageClient();
      const avatar = await storage.getFilePreview("profile", user.prefs.avatar, size, size);
